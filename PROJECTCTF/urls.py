"""PROJECTCTF URL Configuration

The `urlpatterns` list routes URLs to views. For more information please see:
    https://docs.djangoproject.com/en/3.1/topics/http/urls/
Examples:
Function views
    1. Add an import:  from my_app import views
    2. Add a URL to urlpatterns:  path('', views.home, name='home')
Class-based views
    1. Add an import:  from other_app.views import Home
    2. Add a URL to urlpatterns:  path('', Home.as_view(), name='home')
Including another URLconf
    1. Import the include() function: from django.urls import include, path
    2. Add a URL to urlpatterns:  path('blog/', include('blog.urls'))
"""
from django.contrib import admin
from django.urls import path,include
from django.conf.urls.static import static
from django.conf import settings

urlpatterns =[
    path('admin/', admin.site.urls),
    path('',include('CTF.urls')),
    path('login/',include('logindemo.urls')),
    path('register/',include('registerdemo.urls')),
    path('about/',include('landpage.urls')),
    #path('stegnography/',include('stegnography.urls')),
    #path('pwn/',include('pwn.urls')),
    path('cryptography/',include('cryptography.urls')),
    path('web/',include('web.urls')),
    #path('binary/',include('binary.urls')),
    path('forensic/',include('Forensic.urls')),
    path('reverse/',include('reverse.urls')),
    path('misc/',include('misc.urls')),
]

urlpatterns+=static(settings.MEDIA_URL,document_root=settings.MEDIA_ROOT)

